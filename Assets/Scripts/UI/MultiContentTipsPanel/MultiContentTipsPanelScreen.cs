...
#pragma warning restore CS1998
        {
            ...
            else if (mParam.tipType == MultiContentTipsType.FullScreenSpriteContent)
            {
                ...
                ETTool.RemoveAll(mCtrl.go_OpenUrl);
                if (!string.IsNullOrEmpty(mParam.str_OpenUrl))
                {
                    mCtrl.go_OpenUrl.SetActive(true);
                    ETTool.Add(mCtrl.go_OpenUrl,EventTriggerType.PointerClick, (arg0) =>
                    {
                        Debug.Log(mParam.str_OpenUrl);
                        Extension.OpenURL(mParam.str_OpenUrl);
                    });
                }
            }
            ...
            {
                ETTool.Add(mCtrl.go_Background, EventTriggerType.PointerClick, (arg) => { mCtrl.StartCoroutine(mCtrl.Close(mParam.tipType, mParam.closeCallBack)); });
            }
        }
        ...
        private void InitStringTableList()
        {
            ...
            {
                Transform dataParent= pool.Spawn(stringTableListItem, mCtrl.trans_StringTableItems).UIPoolSpawnedResetY()
                    .UIPoolSpawnedSetLast();
                for (int j = 0; j < mParam.stringTableListContents[i].Count; j++)
                {
                    pool.Spawn(stringTableListItemTxt, dataParent).UIPoolSpawnedResetY()
                        .UIPoolSpawnedSetLast().GetComponent<TMP_Text>().text = mParam.stringTableListContents[i][j];
                }
            }
        }

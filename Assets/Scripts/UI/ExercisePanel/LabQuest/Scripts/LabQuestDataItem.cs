...
public class LabQuestDataItem : SerializedMonoBehaviour
{
    ...
    public LabQuestDataItem Init(Question quest, int Index)
    {
        ...
        {
            ChoiceToggle.Add(spawnPool.Spawn(OptionPrefab, toggleParent.transform).UIPoolSpawnedResetY().GetComponent<LabQuestDataOption>().InitOption(quest.optionIsSprite ? (Choice) quest.spriteChoices[i] : (Choice) quest.txtChoices[i]));
